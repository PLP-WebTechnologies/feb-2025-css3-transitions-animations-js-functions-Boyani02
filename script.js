document.addEventListener('DOMContentLoaded', function() {
    ...
    // Animate featured images on page load
    featuredImages.forEach(img => {
        img.classList.add('fade-in');
    });

    // Store and retrieve favorite cuisine using localStorage
    const cuisineSelect = document.getElementById('cuisine');
    const savedCuisine = localStorage.getItem('favoriteCuisine');
    if (savedCuisine) {
        cuisineSelect.value = savedCuisine;
    }

    cuisineSelect.addEventListener('change', function() {
        localStorage.setItem('favoriteCuisine', this.value);
    });

    // Animate button and toggle offers
    toggleButton.addEventListener('click', function() {
